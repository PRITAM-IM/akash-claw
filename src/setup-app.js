...
(function () {
  ...
  function renderAuth(groups) {
    ...
    if (!advancedToggle) {
      ...
      // Insert before authChoiceEl in *its* parent (more robust across markup changes).
      // If we can't find a sane parent, just append to authGroupEl's parent.
      var parent = (authChoiceEl && authChoiceEl.parentNode) ? authChoiceEl.parentNode : authGroupEl.parentNode;
      try {
        if (parent) parent.insertBefore(advancedToggle, authChoiceEl || null);
      } catch (_e) {
        try { if (authGroupEl.parentNode) authGroupEl.parentNode.appendChild(advancedToggle); } catch (_e2) {}
      }
    }
